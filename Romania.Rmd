...
romania_corona_wide_format <- romania_corona %>% 
  ...
  mutate(active = cumsum(confirmed - (death + recovery)))

# long format of data
romania_corona <- romania_corona_wide_format %>% 
  pivot_longer(14:17, names_to = "type", values_to = "cases")
...
recovered        <- romania_corona %>% filter(type == "recovery") 
